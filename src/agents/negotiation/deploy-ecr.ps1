...
Write-Host ""
Write-Host "🏗️  Step 3: Building Docker image..." -ForegroundColor Cyan
docker build -t "${ECR_REPOSITORY_NAME}:${IMAGE_TAG}" .

# Step 4: Tag the image
Write-Host ""
Write-Host "🏷️  Step 4: Tagging Docker image..." -ForegroundColor Cyan
docker tag "${ECR_REPOSITORY_NAME}:${IMAGE_TAG}" "$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/${ECR_REPOSITORY_NAME}:${IMAGE_TAG}"

# Step 5: Push to ECR
Write-Host ""
Write-Host "⬆️  Step 5: Pushing image to ECR..." -ForegroundColor Cyan
